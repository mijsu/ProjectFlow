...
import { orderBy, limit, where } from "firebase/firestore"; // Ensure 'where' is imported
...
export default function RecentActivity() {
  ...
  return (
    <Card className="lg:col-span-2 bg-slate-950 border-slate-800 h-[500px] flex flex-col">
      ...
      <CardContent className="flex-1 overflow-hidden">
        <div className={`space-y-4 h-full ${activities?.length > 6 ? 'overflow-y-auto pr-2' : ''}`}>
          {activities?.length === 0 ? (
            ...
            activities?.map((activity) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center mt-1 ${colorClasses}`}
                  >
